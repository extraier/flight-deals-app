...
async function main() {
  ...
  // ─── Translation pass: fill in 繁體中文 for any new tweet IDs ───
  // Detect tweets whose ID isn't yet in the translations dict (i.e. they were
  // added by the upstream data refresh and we haven't translated them yet).
  // Batch-translate via Google Translate free endpoint (no API key) with
  // ticker/amount placeholders so $AAPL, $1.5B, +3.2% don't get mangled.
  const STALE_TWEET_AGE_DAYS = 30;
  const nowMs = Date.now();
  const needsTranslation = [];
  for (const tw of tweets) {
    if (translations[tw.id]) continue;             // already done
    if (!tw.text || !tw.text.trim()) continue;     // empty
    if (tw.text.length < 4) continue;              // too short
    // Drop ancient tweets (>30 days) — they're already off the visible page
    // (page slice(0,80) by displayTime) and adding 100+ backlog translations
    // on every refresh is wasteful.
    const twMs = tw.createdAt ? Date.parse(tw.createdAt) : nowMs;
    if (Number.isFinite(twMs) && (nowMs - twMs) > STALE_TWEET_AGE_DAYS * 86400_000) continue;
    needsTranslation.push({ id: tw.id, text: tw.text });
  }
  if (needsTranslation.length) {
    console.log(`[serenity] translating ${needsTranslation.length} new tweets…`);
    ...
    console.log(`[serenity] translated ${merged}/${needsTranslation.length} tweets`);
    if (merged > 0) {
      // Persist updated translations so future runs carry them forward.
      try {
        fs.writeFileSync(TRANSLATIONS_PATH, JSON.stringify(translations, null, 2));
        console.log(`[serenity] wrote ${TRANSLATIONS_PATH} (${Object.keys(translations).length} entries)`);
      } catch (err) {
        console.warn(`[serenity] could not persist ${TRANSLATIONS_PATH}: ${err.message}`);
      }
    }
  } else {
    console.log('[serenity] no new tweets to translate');
  }
